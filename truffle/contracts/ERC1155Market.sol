...
//we waant to create a modifier to check if the caller is the item owner
    modifier onlyItemOwner (address tokenAddress, uint256 tokenId) {
        require(IERC1155(nftContract).balanceOf(msg.sender,tokenId) >=1 , "lister is not owner of NFT anymore" );
        _;
    }

    //modifier which checks if the item actuall does exist
    modifier itemExists(uint256 id){
        require(id <= _itemIds && idToMarketItem[id].itemId == id, "could not find item");
        _;
    }
 ...
 */
function createMarketItem(address nftContract, uint256 tokenId, uint category) onlyItemOwner(nftContract, tokenId) public nonReentrant {
    _itemIds.increment();
    uint256 itemId = _itemIds.current();
    

    //One-to-one mapping itemId to item
    idToMarketItem[itemId] =  MarketItem(itemId, nftContract, tokenId,0,ItemCategories(category), false, address(msg.sender),address(msg.sender));
 ...
 */
  ...
  function createMarketListing(uint256 itemId, uint256 price) public itemExists(itemId) nonReentrant {
  ...
  function cancelMarketListing(uint256 itemId) public itemExists(itemId) isForSale(itemId) nonReentrant {
    ...
    //Log creation of Market Listing
    emit MarketListingCanceled(itemId,nftContract,tokenId,msg.sender,price);
  }
 ...
 */
   ...
   function createMarketSale(address nftContract, uint256 itemId) itemExists(itemId) isForSale(itemId) public nonReentrant {
    ...
    require(IERC1155(nftContract).balanceOf(lister,tokenId) >=1 , "lister is not owner of NFT anymore" );
    ...
    gcoToken.transferFrom(address(this),owner,listingPrice);
     emit MarketSaleCreated(itemId,nftContract,tokenId,lister,msg.sender,price);
  }
