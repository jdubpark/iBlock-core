...
describe('Marketplace', async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    NFTMarketFactory = await ethers.getContractFactory('NFTMarket')
  ...
  it('updates items with updateItemDetails()', async () => {
    ...
    const name = '0x1010657320537765617465720000000000000000000000000000000000000000'
    const price = 50
    const urls = ['none']
    const quant_by_size_and_location = [['1'], ['2'], ['3'], ['4'], ['5']]
    const createdItem = await instance.createItem(name, price, urls, quant_by_size_and_location, { from: accounts[0] })
  ...
  it('orderSubmitted operates correctly', async () => {
    ...
    const tx = await instance.orderSubmitted(0, [[[3], [3], [3], [3], [3]], [[0], [1], [2], [0], [0]]], { from: DLab_address })
  ...
  it('adds location', async () => {
    ...
    const locations = await instance.getLocations.call()
  ...
  it('deletes location', async () => {
    ...
    const second_location = await instance.locations.call(1)
  ...
  it('changes location phrase', async () => {
    ...
    const old_phrase = await instance.locations.call(0)
    ...
    const changed_phrase = await instance.locations.call(0)
  ...
  it('changes lowInventory threshold', async () => {
    ...
    const set_limit = await instance.low_inventory_limit.call()
  ...
  it('resets reorder amount', async () => {
    ...
    // pull limit from contract and confirm they match
    const set_limit = await instance.reorder_amount.call()
    assert.equal(new_limit, set_limit, 'the limit was not set correctly')
  })
})
